  ...
  let ray_hits_sphere (sphere: t) (ray: Ray.t) (t_min: float) =
    ...
    if discriminant < 0. then
      ...
      if t < 0.0 then
        ...
        let outward_normal = Vec3.vec_mul_scalar (Vec3.vec_sub point sphere.center) (1.0 /. sphere.radius) in
        let front_face = Vec3.vec_dot ray.direction outward_normal < 0.0 in
        let normal = if front_face then outward_normal else Vec3.vec_neg outward_normal in
        Some {HitRecord.t; point; normal; front_face; material = sphere.material};;
