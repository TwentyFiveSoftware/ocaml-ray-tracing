...
module Camera = struct
  ...
  let new_camera look_from look_at fov width height =
    ...
    let right = Vec3.normalize (Vec3.vec_cross {x = 0.0; y = 1.0; z = 0.0} forward) in
    ...
    let upper_left_corner = Vec3.vec_add 
      (Vec3.vec_add 
        (Vec3.vec_sub look_from (Vec3.vec_mul_scalar horizontal_direction 0.5)) 
  ...
  let get_camera_ray camera u v =
    let target = Vec3.vec_sub (Vec3.vec_add camera.upper_left_corner (Vec3.vec_mul_scalar camera.horizontal_direction u)) 
                        (Vec3.vec_mul_scalar camera.vertical_direction v) in
    {Ray.origin = camera.look_from; direction = Vec3.vec_sub target camera.look_from};;
